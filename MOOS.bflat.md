...
## Experimental:Build MOOS Flattened

### 1.Flatten MOOS project:
```
bflata flatten-all D:\Repos\MOOS\MOOS\MOOS.csproj -h:d:\repos\moos
```
There would be a MOOS.flat folder appear in current directory, into which all codes and dependencies of MOOS have been extracted, and a `moos.bfa` file generated by BFlatA with references to all extracted codes, dependecies and building args of the original project, which you could use BFlatA to build directly. Here, `flatten-all` copies the native libs (instead, `flatten` leave depdencies as they are while writing their path references to the .bfa file), and  `-h:d:\repos\moos` specifies the solution home of MOOS as to locate dependencies, according to MOOS configuration.

### 2.Build the flattened MOOS project from MOOS.flat folder. 
Here it is supposed that BFlatA will be launched in the parent folder of MOOS.flat. (and you may choose another path)
Save the following text to a MOOS.flat.bfa file.

```
# BFlatA verb and project to build
## these two lines must present at the start in order.
## one .bfa file is allowed following the verb to be included for building, as is equivalent to -inc:<.bfa file>.
build
MOOS.flat\moos.bfa
...
-poa:"'D:\Program Files (x86)\VMware\VMware Player\vmplayer.exe' '$(MSBuildStartupDirectory)\Tools\VMWare\MOOS\MOOS.flat.vmx'"
```
### 3.Build the flattened project
Run this in commmand line, will initiate the build process.
```
